...
%	Véronique Larrieu and Triet Nguyen                                                                                    %       
%                                                                                                                                                       %
...
\setmarginsrb{2.9cm}{1.5cm}{2.9cm}{1cm}{\headheight}{\headsep}{\footheight}{\footskip}
...
\begin{figure}[!htb]
\begin{center}
\includegraphics[scale=0.6]{img/densite.pdf}
...
\begin{figure}[!htb]
\begin{center}
\includegraphics[scale=0.6]{img/var90somme.pdf}
\caption{$VaR_{90\%}(X)+VaR_{90\%}(Y) \leq VaR_{90\%}(X+Y)$}
\label{var90somme}
\end{center}
\end {figure}

\subsection{Mesures de d\'ependance}
Il y a maintes façons de mesurer la d\'ependance entre deux variables al\'eatoires. Nous pr\'esentons ici le tau de Kendall
...
les couples $(X_j, Y_j)$ ont la même copule $C$. \\
...
En traçant 10000 r\'ealisations des composantes d'un couple $(U,V)$ de loi jointe $C_\alpha$ (pour $\alpha=1,2,3,4$), on obtient les graphiques suivants.

\begin{figure}[!htb]
\begin{center}
\includegraphics[scale=0.6]{img/simulation1234.pdf}
...
climatiques adapt\'es \`a la r\'eassurance d'\'ev\`enement catastrophe (tempête, vague de froid,\dots) bas\'e sur un indice climatique
...
Enfin, nous avons trac\'e les histogrammes (cf. figure \ref{histogramme}) \`a pas fixe et \`a même effectif du payoff correspondant
